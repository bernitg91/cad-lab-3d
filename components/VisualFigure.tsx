...
export function VisualFigure({ visual, href, className = "", sizes = "(max-width: 640px) 90vw, 30vw", caption }: { visual: Visual; href?: string; className?: string; sizes?: string; caption?: string }) {
  const picture = <Image src={visual.image} alt={visual.alt} width={visual.width} height={visual.height} sizes={sizes} className="cad-photo-image" loading="lazy" />;
