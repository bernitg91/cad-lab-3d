...
const tools = [
  ...
  ["Checklist FDM", "Una última revisión de archivo, máquina, material y acabado.", "/checklist-impresion-3d", "✓"]
];
...
const benchPieces = [
  {
    ...
    image: "/images/impresion-3d-personalizada/adaptador-aro-funcional-fdm.jpg",
    alt: "Mano sosteniendo un adaptador negro impreso en 3D",
  ...
  },
  {
    ...
    alt: "Mano sosteniendo un panel gris impreso en 3D con relieves",
  ...
  },
  {
    ...
    alt: "Soporte impreso en 3D con varios relojes",
    note: "La forma y los apoyos están fotografiados; faltan los datos del perfil de impresión.",
  ...
  },
  {
    ...
    alt: "Pieza decorativa azul impresa en 3D sobre una mesa de madera",
    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-14 lg:px-8 lg:py-16">
          <div>
            ...
            <p className="mt-5 flex max-w-xl gap-3 border-l-2 border-orange-600 pl-4 text-sm leading-6 text-slate-600">
              Si una medida, un material o un ensayo no están documentados, lo indico. Una fotografía ayuda a observar; no convierte una pieza en un caso validado.
            </p>
          ...
          <figure className="border border-slate-300 bg-[#eef2f6] p-3 sm:p-4">
            <div className="relative aspect-[4/3] overflow-hidden bg-slate-200">
              <Image
                src="/images/impresion-3d-personalizada/adaptador-aro-funcional-fdm.jpg"
                alt="Mano sosteniendo un adaptador negro impreso en 3D delante de un teclado"
                fill
                priority
                fetchPriority="high"
            ...
            <figcaption className="bg-[#091625] p-5 text-white sm:p-6">
              <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-cyan-300">Ficha de banco · fotografía propia</p>
              <h2 className="mt-2 font-display text-3xl font-black leading-none">Adaptador de pequeño formato</h2>
              <dl className="mt-5 grid gap-3 text-sm leading-6 sm:grid-cols-3">
                <div><dt className="font-bold text-white">Se ve</dt><dd className="text-slate-300">Geometría, abertura y acabado.</dd></div>
                <div><dt className="font-bold text-white">No consta</dt><dd className="text-slate-300">Material, holgura y repetibilidad.</dd></div>
                <div><dt className="font-bold text-white">Siguiente prueba</dt><dd className="text-slate-300">Medir el diámetro y repetir la pieza.</dd></div>
              </dl>
            </figcaption>
          </figure>
        </div>
      ...
      <section className="border-y border-slate-200 bg-white py-12 lg:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-[1.25fr_0.75fr]">
            <article className="grid border-t-2 border-slate-950 pt-5 sm:grid-cols-[0.82fr_1.18fr] sm:gap-6">
              <Link href={`/blog/${leadStory.slug}`} className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                <Image src={leadPhoto.image} alt={leadPhoto.alt} fill sizes="(min-width: 1024px) 34vw, 100vw" className="object-cover" />
              </Link>
              <div className="mt-5 sm:mt-0">
                <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.13em] text-teal-800">{leadStory.category} · foto propia</p>
                <h3 className="mt-3 font-display text-4xl font-black leading-[0.98] text-slate-950 sm:text-5xl">
                  <Link href={`/blog/${leadStory.slug}`} className="hover:text-blue-800">{leadStory.title}</Link>
                </h3>
                <p className="mt-4 text-base leading-7 text-slate-600">{leadStory.description}</p>
                <Link className="mt-6 inline-flex border-b-2 border-blue-700 pb-1 text-sm font-black text-blue-800" href={`/blog/${leadStory.slug}`}>Leer la guía →</Link>
              </div>
            </article>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="grid gap-8 lg:grid-cols-[0.7fr_1.3fr]">
          <div>
            <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.15em] text-teal-800">Fotos de la mesa de trabajo</p>
            ...
            <p className="mt-5 max-w-lg text-base leading-7 text-slate-600">Estas imágenes son propias. Con ellas se puede hablar de forma y acabado visible; no de resistencia, material o precisión cuando esos datos no se conservaron.</p>
            <Link className="mt-6 inline-flex border-b-2 border-blue-700 pb-1 text-sm font-black text-blue-800" href="/casos-practicos-impresion-3d">Abrir la galería completa →</Link>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            {benchPieces.map((piece) => (
              <Link key={piece.href} href={piece.href} className="group border-t-2 border-slate-950 pt-3">
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                  <Image src={piece.image} alt={piece.alt} fill sizes="(min-width: 1024px) 29vw, (min-width: 640px) 50vw, 100vw" className="object-cover transition duration-300 group-hover:scale-[1.02]" />
                </div>
                <h3 className="mt-4 font-display text-2xl font-black leading-none text-slate-950 group-hover:text-blue-800">{piece.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600">{piece.note}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-slate-200 bg-[#eef2f6] py-12 lg:py-16">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-8">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">Es un cuaderno público de aprendizaje. Las guías ordenan lo que ya se puede explicar; las fotos muestran piezas reales; los huecos de información se dejan visibles para no convertir una suposición en un resultado.</p>
          </div>
