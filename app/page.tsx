...
export default function HomePage() {
  ...
  const featuredSlugs = ["disenar-encajes-clips-impresion-3d", "pruebas-tolerancia-fdm", "preparar-archivo-step-compartir-cad"];
  ...
  return <>
    ...
    <section className="cad-reading-section"><div className="cad-section">
      ...
      <div className="cad-reading-grid">{selected.map(article=>article&&<article key={article.slug} className="cad-reading-card"><p className="cad-eyebrow">{article.category}</p><h3><Link href={`/blog/${article.slug}`}>{article.title}</Link></h3><p>{article.description}</p><Link href={`/blog/${article.slug}`} className="cad-card-link">Leer artículo <span aria-hidden="true">↗</span></Link></article>)}</div>
